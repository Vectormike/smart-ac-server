...
export class DeviceReportValidator {
  CreateDeviceReportDto = createValidator((Joi) => {
    return {
      ...
      temperature: Joi.string().required().trim().error(new Error('Temperature is required')),
      humidity: Joi.string().required().error(new Error('Humidity is required')),
      carbonMonoxide: Joi.string().required().error(new Error('CO level is required')),
